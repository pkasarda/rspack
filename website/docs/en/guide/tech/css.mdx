...
By default, built-in CSS support leaves `exportType` unset and uses `link` behavior, which emits CSS as stylesheet output. You can configure `module.parser.css.exportType` or the parser options for `css/auto`, `css/global`, and `css/module` to export CSS through JavaScript instead.
